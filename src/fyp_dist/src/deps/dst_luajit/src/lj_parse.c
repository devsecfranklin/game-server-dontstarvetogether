...
static void fs_fixup_k(FuncState *fs, GCproto *pt, void *kptr)
{
  ...
  // Hack to print out constant information..
  // printf("%i out of %i\n", fs->nkgc, BCMAX_D+1);
  checklimitgt(fs, fs->nkn, BCMAX_D+1, "constants 1");
  checklimitgt(fs, fs->nkgc, BCMAX_D+1, "constants 2"); // this fails
  setmref(pt->k, kptr);
  pt->sizekn = fs->nkn;
  pt->sizekgc = fs->nkgc;
  kt = fs->kt;
