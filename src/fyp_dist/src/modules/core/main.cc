...
static PoutFactory log("[fyp:core:main]");
...
FYP_API void mod_main() {
    ...
    if (C.options.verbose) {
        log.green() << "Going with these paths:" << std::endl;
        log.green() << "Root:    " << C.path_root() << std::endl;
        log.green() << "Bin:     " << C.path_bin() << std::endl;
        log.green() << "Data:    " << C.path_data() << std::endl;
        log.green() << "Mods:    " << C.path_mod() << std::endl;
        log.green() << "UgcMods: " << C.path_mod_ugc() << std::endl;
        log.green() << "Klei:    " << C.path_klei() << std::endl;
    }
/*
    if (C.options.update) {
        Updater update(C.path_mod_ugc(), C.path_mod());
        for (auto const& p : update.hash_ugcmods())
            log.green() << p.hash << " " << p.path << std::endl;
    }
    */
    hook_all();
}
...
void* handle_lua_newstate(void* f, void* ud) {
    ...
    log.green() << "Jit turned on, enter jit_on() or jit_off() to set" << std::endl;
    C.lua.luaJIT_setmode(L, 0, LUAJIT_MODE_ENGINE | LUAJIT_MODE_ON);

    log.cyan() << "lua_State created, loading fyp_modules" << std::endl;
    load_fyp_modules();
...
void hook_all() {
    ...
    if (C.options.verbose) {
        log.green() << "CWD: " << fs::current_path() << std::endl;
    }
    log.green() << "Loading luajit..." << std::endl;
    ...
    if (C.options.verbose) {
        log.cyan() << format("luajit loaded at: 0x{:x}", (size_t)h_luajit) << std::endl;
        log.cyan() << format("luajit luaL_newstate at: 0x{:x}", (size_t)C.lua.luaL_newstate)
                   << std::endl;
        log.cyan() << format("luajit lua_newstate at: 0x{:x}", (size_t)C.lua.lua_newstate)
                   << std::endl;
        log.cyan() << format("luajit luaJIT_setmode at: 0x{:x}", (size_t)C.lua.luaJIT_setmode)
                   << std::endl;
    }
    ...
    for (auto& sig : SIGS_ALL) {
        ...
        if (fname == "lua_newstate") {
            if (C.options.verbose)
                log.cyan() << format("{:<22}[0x{:016x}] -> 0x{:016x} [logic modified]"
                        , sig.function_name
                        , address
        ...
        if (fname == "dst_lua_debug_getsize") {
            if (C.options.verbose)
                log.green() << "Installing debug:getsize stub" << std::endl;
        ...
        if (jitaddress == 0) {
            ...
            log.red() << format("LuaJIT export not found: {:<22}[0x{:016x}] -> 0x{:016x}", sig.function_name, address, jitaddress)
                      << std::endl;
        }

        if (C.options.verbose)
            log.cyan() << format("{:<22}[0x{:016x}] -> 0x{:016x}", sig.function_name, address, jitaddress)
                       << std::endl;
        ghook(address, jitaddress);
    }

    if (not_found.size() == 0)
        log.green() << "Found all signatures" << std::endl;
    else {
        log.red() << format("Missed {} signatures out of {}", not_found.size(), numsigs)
                  << std::endl;
        for (auto& sig : not_found)
            log.cyan() << "Missed: " << sig.function_name << std::endl;
    }
...
static int jit_off(lua_State* L) {
    ...
    log.red() << "Jit: OFF" << std::flush << std::endl;
    return 1;
}
