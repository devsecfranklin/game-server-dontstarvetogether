...
namespace libpe
{
	...
	extern "C" inline ILIBPEAPI LIBPEINFO __cdecl GetLibInfo() {
		return { LIBPE_VERSION_WSTR, LIBPE_VERSION_ULONGLONG };
	}

	//Checking overflow at summing two DWORD_PTR.
	constexpr inline bool IsSumOverflow(DWORD_PTR dwFirst, DWORD_PTR dwSecond) {
	...
	{
	public:
		auto inline ParsePE(LPCWSTR pwszFile) -> int override;
		auto inline ParsePE(std::span<const std::byte> spnFile) -> int override;
		auto inline LoadPe(LPCWSTR pwszFile) -> int override;
		auto inline LoadPe(std::span<const std::byte> spnFile) -> int override;
		[[nodiscard]] auto inline GetFileInfo()const->PEFILEINFO override;
		[[nodiscard]] auto inline GetOffsetFromRVA(ULONGLONG ullRVA)const->DWORD override;
		[[nodiscard]] auto inline GetOffsetFromVA(ULONGLONG ullVA)const->DWORD override;
		[[nodiscard]] auto inline GetMSDOSHeader() -> IMAGE_DOS_HEADER* override;
		[[nodiscard]] auto inline GetRichHeader() -> PERICHHDR_VEC* override;
		[[nodiscard]] auto inline GetNTHeader() -> PENTHDR* override;
		[[nodiscard]] auto inline GetDataDirs() -> PEDATADIR_VEC* override;
		[[nodiscard]] auto inline GetSecHeaders() -> PESECHDR_VEC* override;
		[[nodiscard]] auto inline GetExport() -> PEEXPORT* override;
		[[nodiscard]] auto inline GetImport() -> PEIMPORT_VEC* override;
		[[nodiscard]] auto inline GetResources() -> PERESROOT* override;
		[[nodiscard]] auto inline GetExceptions() -> PEEXCEPTION_VEC* override;
		[[nodiscard]] auto inline GetSecurity() -> PESECURITY_VEC* override;
		[[nodiscard]] auto inline GetRelocations() -> PERELOC_VEC* override;
		[[nodiscard]] auto inline GetDebug() -> PEDEBUG_VEC* override;
		[[nodiscard]] auto inline GetTLS() -> PETLS* override;
		[[nodiscard]] auto inline GetLoadConfig() -> PELOADCONFIG* override;
		[[nodiscard]] auto inline GetBoundImport() -> PEBOUNDIMPORT_VEC* override;
		[[nodiscard]] auto inline GetDelayImport() -> PEDELAYIMPORT_VEC* override;
		[[nodiscard]] auto inline GetCOMDescriptor() -> PECOMDESCRIPTOR* override;
		void inline Clear()override;
		void inline Destroy()override;
	private:
		void ClearAll();
		[[nodiscard]] auto inline GetBaseAddr()const->DWORD_PTR;
		[[nodiscard]] auto inline GetDataSize()const->ULONGLONG;
		[[nodiscard]] auto inline GetDosPtr()const->const IMAGE_DOS_HEADER*;
		[[nodiscard]] auto inline GetDirEntryRVA(DWORD dwEntry)const->DWORD;
		[[nodiscard]] auto inline GetDirEntrySize(DWORD dwEntry)const->DWORD;
		[[nodiscard]] auto inline GetImageBase()const->ULONGLONG;
		[[nodiscard]] auto inline GetSecHdrFromName(LPCSTR lpszName)const->PIMAGE_SECTION_HEADER;
		[[nodiscard]] auto inline GetSecHdrFromRVA(ULONGLONG ullRVA)const->PIMAGE_SECTION_HEADER;
		template<typename T>
		[[nodiscard]] auto inline GetTData(ULONGLONG ullOffset)const->T;
		template<typename T>
		[[nodiscard]] auto inline IsPtrSafe(T tAddr, bool fCanReferenceBoundary = false)const->bool;
		[[nodiscard]] auto inline PtrToOffset(LPCVOID lp)const->DWORD;
		[[nodiscard]] auto inline RVAToOffset(ULONGLONG ullRVA)const->DWORD;
		[[nodiscard]] auto inline RVAToPtr(ULONGLONG ullRVA)const->LPVOID;
		bool inline ParseMSDOSHeader();
		bool inline ParseRichHeader();
		bool inline ParseNTFileOptHeader();
		bool inline ParseDataDirectories();
		bool inline ParseSectionsHeaders();
		bool inline ParseExport();
		bool inline ParseImport();
		bool inline ParseResources();
		bool inline ParseExceptions();
		bool inline ParseSecurity();
		bool inline ParseRelocations();
		bool inline ParseDebug();
		bool inline ParseArchitecture();
		bool inline ParseGlobalPtr();
		bool inline ParseTLS();
		bool inline ParseLCD();
		bool inline ParseBoundImport();
		bool inline ParseIAT();
		bool inline ParseDelayImport();
		bool inline ParseCOMDescriptor();
	private:
	...
	extern "C" inline ILIBPEAPI Ilibpe * __cdecl CreateRawlibpe() {
		return new Clibpe();
	}

	inline auto Clibpe::ParsePE(LPCWSTR pwszFile)->int
	{
	...
	auto inline Ilibpe::FlatResources(const PERESROOT& stResRoot)->PERESFLAT_VEC
	{
