...
static PoutFactory plog("[fyp:maphack]");
...
static int hack_map(lua_State* L) {
    ...
    plog.red() << "Hacks: maphack" << std::endl;
    return 1;
}

extern "C" FYP_API void mod_init() {
    auto& C = Core::get_instance();
    plog.green() << "Installing maphack lua command" << std::endl;

    auto& bounds = C.get_scan_bounds();
    plog.green() << "Start: " << std::hex << bounds.start << " end: " << bounds.end << std::endl;
    size_t address = find_signature(bounds.start, bounds.end, sig_maphack);
    if (0 == address) {
        plog.red() << "Failed to find patch location" << std::endl;
        return;
    }
    maphack_patch_addr = address + 7;
    plog.green() << "address is: " << std::hex << maphack_patch_addr << std::endl;
