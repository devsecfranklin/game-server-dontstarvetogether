...
static PoutFactory plog("[fyp:core:core]");
...
Core::~Core() {
    plog.green() << "Cleanup time..." << std::endl;
    ...
    plog.red() << "ITS JUST A FLESH WOUND, HAVE AT YOU" << std::endl;
}
...
void Core::init() {
    ...
    if (perr)
        plog.red() << "Ini parser failed, does the file exist? : " << perr
                  << std::endl;
