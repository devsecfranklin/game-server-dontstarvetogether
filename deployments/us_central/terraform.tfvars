# --- GENERAL --- #
location = "Central US"
# subscription_id       = "" # TODO <- uncomment and populate with proper subscription ID
resource_group_name   = "mna-usce-core-rg-ngfw"
name_prefix           = "mna-usce-core-"
create_resource_group = false
tags = {
  "DataClassification" = "Private"
  "Environment"        = "Core"
  "ProjectCode"        = "50104"
  "ResourceManagedBy"  = "Chris Misson"
  "ResourceOwnedBy"    = "Mark Baltz"
  "SecurityProfile"    = "Internal"
}
enable_zones = true



# --- VNET PART --- #
vnets = {
  "vnet-ntw" = {
    create_virtual_network = false
    address_space          = ["10.74.0.0/20"]
    network_security_groups = {
      "mna-usce-core-nsg-ngfw" = {
        rules = {
          vmseries_mgmt_allow_inbound = {
            priority                   = 100
            direction                  = "Inbound"
            access                     = "Allow"
            protocol                   = "Tcp"
            source_address_prefixes    = ["134.238.135.137", "134.238.135.14"] # TODO <- whitelist public IPs used to management
            source_port_range          = "*"
            destination_address_prefix = "10.74.1.0/27"
            destination_port_ranges    = ["22", "443"]
          }
        }
      }
      "mna-usce-core-nsg-priv"       = {}
      "mna-usce-core-nsg-pa-untrust" = {}
    }
    route_tables = {
      "mna-usce-core-mgmt-routes" = {
        routes = {
          "private_blackhole" = {
            address_prefix = "10.74.0.64/27"
            next_hop_type  = "None"
          }
          "public_blackhole" = {
            address_prefix = "10.74.4.0/23"
            next_hop_type  = "None"
          }
        }
      }
      "mna-usce-core-priv-routes" = {
        routes = {
          "default" = {
            address_prefix         = "0.0.0.0/0"
            next_hop_type          = "VirtualAppliance"
            next_hop_in_ip_address = "10.74.0.73"
          }
          "mgmt_blackhole" = {
            address_prefix = "10.74.1.0/27"
            next_hop_type  = "None"
          }
          "public_blackhole" = {
            address_prefix = "10.74.4.0/23"
            next_hop_type  = "None"
          }
        }
      }
      "mna-usce-core-untrust-routes" = {
        routes = {
          "mgmt_blackhole" = {
            address_prefix = "10.74.1.0/27"
            next_hop_type  = "None"
          }
          "private_blackhole" = {
            address_prefix = "10.74.0.64/27"
            next_hop_type  = "None"
          }
        }
      }
    }
    create_subnets = false
    subnets = {
      "mna-usce-core-sub-ngfw" = {
        network_security_group = "mna-usce-core-nsg-ngfw"
        route_table            = "mna-usce-core-mgmt-routes"
      }
      "mna-usce-core-sub-priv" = {
        network_security_group = "mna-usce-core-nsg-priv"
        route_table            = "mna-usce-core-priv-routes"
      }
      "mna-usce-core-sub-pa-untrust" = {
        network_security_group = "mna-usce-core-nsg-pa-untrust"
        route_table            = "mna-usce-core-untrust-routes"
      }
    }
  }
}


# --- LOAD BALANCING PART --- #
load_balancers = {
  "lb-public" = {
    vnet_name                         = "vnet-ntw"
    network_security_group_name       = "mna-usce-core-nsg-pa-untrust"
    network_security_allow_source_ips = ["134.238.135.137", "134.238.135.14"] # TODO <- whitelist public IPs allowed to connect to public LB and Firewalls' PIPs
    avzones                           = ["1", "2", "3"]

    frontend_ips = {
      "palo-lb-app1-pip" = {
        create_public_ip = true # TODO <- set this to false when reusing an existing PIP
        # public_ip_resource_group = "" # TODO <- in case an existing PIP is used, provide it's name and RG
        # public_ip_name           = ""
        rules = {
          "balanceHttp" = {
            protocol = "Tcp"
            port     = 80
          }
          "balanceHttps" = {
            protocol = "Tcp"
            port     = 443
          }
        }
      }
    }
  }
  "lb-private" = {
    frontend_ips = {
      "ha-ports" = {
        vnet_name          = "vnet-ntw"
        subnet_name        = "mna-usce-core-sub-priv"
        private_ip_address = "10.74.0.73"
        rules = {
          HA_PORTS = {
            port     = 0
            protocol = "All"
            zones    = ["1", "2", "3"]
          }
        }
      }
    }
  }
}

vmseries_version  = "10.1.8"
vmseries_vm_size  = "Standard_DS3_v2"
vmseries_sku      = "byol"
vmseries_password = "123QWEasd" # TODO <- adjust vmseries initial password, or comment out for autogenerated one
vmseries = {
  "azeeuwevm001" = {
    bootstrap_options = "type=dhcp-client" # TODO <- adjust vmseries bootstrap options
    vnet_name         = "vnet-ntw"
    avzone            = 1
    interfaces = [
      {
        name               = "management"
        subnet_name        = "mna-usce-core-sub-ngfw"
        create_pip         = true
        private_ip_address = "10.74.1.5"
      },
      {
        name                 = "public"
        subnet_name          = "mna-usce-core-sub-pa-untrust"
        backend_pool_lb_name = "lb-private"
        private_ip_address   = "10.74.4.25"
      },
      {
        name                 = "private"
        subnet_name          = "mna-usce-core-sub-priv"
        backend_pool_lb_name = "lb-public"
        create_pip           = true
        private_ip_address   = "10.74.0.71"
      }
    ]
  }
  "azeeuwevm002" = {
    bootstrap_options = "type=dhcp-client" # TODO <- adjust vmseries bootstrap options
    vnet_name         = "vnet-ntw"
    avzone            = 2
    interfaces = [
      {
        name               = "nic-management"
        subnet_name        = "mna-usce-core-sub-ngfw"
        create_pip         = true
        private_ip_address = "10.74.1.6"
      },
      {
        name                 = "nic-public"
        subnet_name          = "mna-usce-core-sub-pa-untrust"
        backend_pool_lb_name = "lb-private"
        private_ip_address   = "10.74.4.26"
      },
      {
        name                 = "nic-private"
        subnet_name          = "mna-usce-core-sub-priv"
        backend_pool_lb_name = "lb-public"
        create_pip           = true
        private_ip_address   = "10.74.0.72"
      }
    ]
  }
}
